"""JSON-string action-envelope example using the provider layer."""
...
def main() -> None:
    ...
    with TemporaryDirectory() as workspace:
        context = ToolContext(invocation_id="prompt-turn", workspace=workspace)
        prepared = executor.prepare_model_interaction(
            adapter,
            context=context,
            include_requires_approval=True,
        )
        print("Action-envelope schema:")
        ...
        provider = OpenAICompatibleProvider(
            ...
            client=_FakeClient(
                {
                    "actions": [
                        {"tool_name": "read_file", "arguments": {"path": "prompt/demo.txt"}}
                    ],
                    "final_response": None,
                }
            ),
        ...
        final_provider = OpenAICompatibleProvider(
            model="demo-model",
            client=_FakeClient({"actions": [], "final_response": "No tool required."}),
        )
