...
def test_run_git_diff_tool_supports_staged_and_ref(
    ...
    staged = _runtime().execute(
        ToolInvocationRequest(tool_name="run_git_diff", arguments={"staged": True}),
        context,
    )
    compared = _runtime().execute(
        ToolInvocationRequest(tool_name="run_git_diff", arguments={"ref": "HEAD~1"}),
        context,
    )

    assert staged.ok is True
    assert staged.output is not None and staged.output["diff_text"] == "diff\n"
    assert compared.ok is True
    assert compared.output is not None and compared.output["diff_text"] == "diff\n"
    assert calls == [
...
def test_run_git_diff_tool_rejects_option_like_refs(
    ...
    result = _runtime().execute(
        ToolInvocationRequest(
            tool_name="run_git_diff",
            arguments={"ref": "--output=/tmp/leak"},
        ),
    ...
    assert result.error.details["exception_message"] == (
        "Git diff ref must not start with '-'."
    )
...
def test_run_git_log_tool_invokes_git_log(
    ...
    assert result.ok is True
    assert result.output is not None and result.output["log_text"] == "abc123 commit\n"
